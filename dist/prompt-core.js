...
function parseFrontMatterBlock(raw) {
	...
	for (const line of String(raw || "").split(/\r?\n/)) {
		...
		if (!key) continue;
		data[key] = parseInlineList(value) ?? unquote(value);
	}
	return data;
}
...
function sectionRanges(body) {
	...
	for (const line of lines) {
		...
		if (!inFence) {
			const title = line.match(/^##[ \t]+(.+?)[ \t]*(?:\r?\n)?$/)?.[1];
			if (title) ranges.push({
				headingStart: offset,
...
function agentSection(body, agent) {
	...
	for (const range of ranges) {
		...
		if (title.includes("same prompt") || title.includes("claude / codex")) fallback = section;
	}
...
//#endregion
export { agentSection, filterPrompts, loadPrompt, promptOutput, promptTextWithOutput, splitFrontMatter };
